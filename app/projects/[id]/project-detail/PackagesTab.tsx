    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {latestCalculation && (
            <Link href={`/calculations/${latestCalculation.id}`} className="btn-secondary text-xs">
      ...
      {packages.length === 0 ? (
        <div className="card p-10 text-center">
          ...
          {latestCalculation && (
            <div className="mt-4">
              <Link href={`/calculations/${latestCalculation.id}`} className="btn-primary text-xs">
                Перейти в расчёт
              </Link>
            </div>
          )}
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      ...
                      {pkg.status !== 'approved' &&
                        (canDecide ? (
                          ...
                          >
                            на подписи: {REVIEW_STAGE_LABELS[stage] ?? 'ревью'}
                          </span>
                        ))}
                      <Link
