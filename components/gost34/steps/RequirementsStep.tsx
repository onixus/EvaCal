...
export default function RequirementsStep({
  ...
  /**
   * Требования, выведенные из этапов расчёта. Мастер их не редактирует, но их
   * замечания блокируют выпуск, поэтому они должны быть видны и подписаны
   * источником — иначе блокировка выглядит беспричинной.
   */
  const ownRequirementCodes = new Set(requirements.map((req) => req.code));
  const derivedRequirements = (review?.requirements || []).filter(
    (req) => !ownRequirementCodes.has(req.code),
  );
      ...
      {derivedRequirements.length > 0 && (
        <div className={`${PANEL_CLASS} space-y-3`}>
          <div>
            ...
            <p className="text-xs text-slate-300 mt-1">
              Собраны из поля «Требования» этапов работ. Правятся в самом расчёте, а не в мастере,
              но проверяются наравне с остальными.
            </p>
          </div>
