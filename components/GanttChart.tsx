          ...
          {stages.map((stage) => {
            ...
            return (
              <div key={stage.id} className="flex items-center gap-3">
                <div className="w-56 shrink-0 truncate text-sm text-slate-700" title={stage.name}>
                ...
                <div className="relative h-6 flex-1 rounded bg-slate-100">
                  <div
                    className={`absolute h-6 rounded ${
                      stage.isApprovalTask ? "bg-amber-400/70 border border-dashed border-amber-500" : ROLE_COLORS[stage.role] ?? "bg-slate-400"
                    }`}
