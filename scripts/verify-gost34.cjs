...
const {
  GOLDEN_SCENARIOS
} = require(root + '/lib/gost34/__tests__/golden/scenarios.ts');
const {
  buildGoldenSnapshot
} = require(root + '/lib/gost34/__tests__/golden/snapshot.ts');
const {
  prepareGost34Document
} = require(root + '/lib/gost34/generation/prepareDocument.ts');
const {
  generateGost34Document
} = require(root + '/lib/gost34/generation/exportDocument.ts');
const {
  exportGost34ToDocx
} = require(root + '/lib/gost34/exporters/docxExporter.ts');
const {
  validateSchemaCoverage
} = require(root + '/lib/gost34/schema/coverage.ts');
const {
  Gost34StructureError,
  UnsupportedGostDocumentTypeError
} = require(root + '/lib/gost34/generation/errors.ts');
const {
  contentDisposition
} = require(root + '/lib/exportResponse.ts');
const {
  CURRENT_GOST34_PROFILE_ID,
  LEGACY_GOST34_PROFILE_ID
} = require(root + '/lib/gost34/standards/index.ts');
const {
  TzAuthorHardFlagsError
} = require(root + '/lib/gost34/llm/tzAuthor/validate.ts');
const {
  buildProjectContext
} = require(root + '/lib/gost34/context/builder.ts');
let assertions = 0,
  docs = 0;
function check(fn) {
  fn();
...
async function run() {
  for (const scenario of GOLDEN_SCENARIOS) {
    check(() => assert.deepEqual(buildGoldenSnapshot(scenario), JSON.parse(fs.readFileSync(root + '/lib/gost34/__tests__/golden/' + scenario.id + '.json', 'utf8'))));
  ...
  for (const value of ['BOGUS', 'constructor', '__proto__']) check(() => assert.throws(() => prepareGost34Document({
    metadataOverride: {
      docType: value
    }
  }), UnsupportedGostDocumentTypeError));
  ...
  check(() => assert(preview.diagnostics.issues.some(i => i.nodeId === 'tz2020-general' && i.kind === 'empty')));
  check(() => assert(preview.baselineAst.sections[0].paragraphs.length > 0));
  check(() => assert.throws(() => prepareGost34Document({
    sectionOverrides: overrides
  }), Gost34StructureError));
  const schema = {
    id: 's',
  ...
  ast.sections = [sec('body', [sec('sub', [sec('deep')])]), ...['А', 'Б'].map(letter => ({
    ...sec(letter),
  ...
  check(() => assert.equal(new Headers({
  ...
  check(() => assert(prepareGost34Document(params, {
    mode: 'preview'
  ...
  check(() => assert(JSON.stringify(funding.ast.sections).includes('Средства Заказчика, оплата по этапам')));
