...
В систему встроены готовые пресеты для быстрого старта с предзаполненными параметрами, нормативными актами и требованиями. Реквизиты продуктов (номера реестров Минцифры/Минпромторга, сертификаты ФСТЭК/ФСБ) централизованы в базе знаний вендоров `lib/gost34/vendors` и сверены с публичными реестрами и вендорскими документами:
...
- **Kaspersky Endpoint Security**: расширенная корпоративная защита рабочих станций и серверов, сертификат ФСТЭК № 4068.
...
- **SIEM и мониторинг ИБ**: MaxPatrol SIEM (реестр № 1143, ФСТЭК № 4980), Kaspersky KUMA (реестр № 9128), взаимодействие с ГосСОПКА (приказы ФСБ № 366-368).
...
- **Пресейл**: логин `presale` / пароль `presale`
- **Архитектор**: логин `architect` / пароль `architect`
- **Ревьювер**: логин `reviewer` / пароль `reviewer`
- **Администратор**: логин `admin` / пароль `admin`

> Пароли по умолчанию актуальны только для локального dev-стенда. В рабочем окружении сид генерирует случайные пароли (см. `credentials.local.txt`) — смените их в `/account` после первого входа.
...
| Документ                                                                           | О чём                                                                        |
| ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| [docs/ROLES_ONBOARDING.md](docs/ROLES_ONBOARDING.md)                               | **Маршрутный лист новичка:** сценарий работы для всех четырёх ролей          |
