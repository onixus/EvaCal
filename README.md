...
Для типовых проектов ИБ предусмотрены отдельные внедренческие шаблоны **SIEM / SOC**, **IDM / IGA** и **NGFW** с полноценными пресейл-опросниками, автоматическим sizing, количественными драйверами трудозатрат и формированием проверяемых требований для ТЗ/ПМИ. Практическая схема применения, формулы sizing, входные данные и критерии приемки описаны в [`docs/IMPLEMENTATION_TEMPLATES.md`](docs/IMPLEMENTATION_TEMPLATES.md).

- **NGFW**: отдельный пресет внедрения периметра и сегментации (`preset-ngfw-implementation`) плюс совместимый комбинированный пресет СЗИ (`preset-ngfw-szi`) для проектов с endpoint-защитой, аудитом хранилищ и ГОСТ-VPN.
...
- **SIEM / SOC**: пресет `preset-siem-soc` для оценки источников событий, EPS, коннекторов, корреляционных правил, опытной эксплуатации и интеграции с ГосСОПКА при применимости.
- **IDM / IGA**: пресет `preset-idm-access-management` для Joiner/Mover/Leaver, ролевой модели, провижининга, рекертификации и интеграции с каталогами/бизнес-системами.
