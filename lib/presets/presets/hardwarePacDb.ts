  ...
  stageTemplates: [
    {
      ...
      requirements:
        'Главный архитектор проекта (ГАП) обязан выполнить расчет тепловыделения, энергопотребления и весовых нагрузок на фальшпол и разработать схемы коммутации SAN/LAN.',
      order: 0,
    },
    {
      ...
      driverFieldKey: 'servers_count',
      requirements:
        'Исполнитель обязан провести входной контроль оборудования с проверкой комплектности, заводских номеров, сертификатов подлинности и гарантийных талонов.',
      order: 1,
    },
    {
      ...
      requirements:
        'Исполнитель обязан установить рельсы, зафиксировать серверы и СХД, проложить оптические и медные патч-корды по кабельным органайзерам и настроить out-of-band управление IPMI/iDRAC.',
      order: 2,
    },
    {
      ...
      driverFieldKey: 'servers_count',
      requirements:
        'Исполнитель обязан развернуть защищенный режим Смоленск/Воронеж и настроить мандатный контроль доступа (Parsec), синхронизацию времени NTP и syslog.',
      order: 3,
    },
    {
      ...
      requirements:
        'Исполнитель обязан настроить пул соединений PgBouncer, синхронную/асинхронную потоковую репликацию и автоматический фейловер и оптимизировать shared_buffers под All-Flash NVMe.',
      order: 4,
    },
    {
      ...
      requirements:
        'Исполнитель обязан настроить расписание полных и инкрементальных бэкапов и дедупликацию, проверить восстановление базы и провести приёмо-сдаточные испытания ПАК по ПМИ.',
      order: 5,
    },
  ],
  riskTemplates: [
