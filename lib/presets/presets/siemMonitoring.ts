  ...
  stageTemplates: [
    {
      ...
      requirements:
        'Исполнитель обязан провести инвентаризацию источников событий, оценить EPS и требования к хранению, разработать схему потоков логирования, выполнить сайзинг платформы SIEM по методике вендора и сформировать перечень сценариев выявления инцидентов (use cases).',
      order: 0,
    },
    {
      ...
      driverFieldKey: null,
      requirements:
        'Исполнитель обязан установить компоненты SIEM по архитектуре вендора и настроить отказоустойчивость, ретенцию и разграничение доступа операторов SOC.',
      order: 1,
    },
    {
      ...
      driverFieldKey: 'event_sources_count',
      requirements:
        'Исполнитель обязан настроить аудит на источниках (ОС, СУБД, СЗИ, сетевое оборудование), подключить их штатными коннекторами и обеспечить нормализацию и категоризацию событий и контроль полноты поступления.',
      order: 2,
    },
    {
      ...
      requirements:
        'Исполнитель обязан разработать парсеры для нестандартных источников, адаптировать и написать правила корреляции под сценарии выявления инцидентов и выполнить тюнинг ложных срабатываний.',
      order: 3,
    },
    {
      ...
      driverFieldKey: null,
      requirements:
        'Исполнитель обязан разработать регламенты выявления и реагирования на инциденты, карточки инцидентов и порядок информирования НКЦКИ через техническую инфраструктуру ГосСОПКА (приказы ФСБ России № 366-368) — для субъектов КИИ.',
      order: 4,
    },
    {
      ...
      driverFieldKey: 'event_sources_count',
      requirements:
        'Исполнитель обязан обеспечить сопровождение опытной эксплуатации, верифицировать сценарии на тестовых атаках, провести обучение операторов и аналитиков и провести приёмочные испытания по ПМИ.',
      order: 5,
    },
  ],
  riskTemplates: [
