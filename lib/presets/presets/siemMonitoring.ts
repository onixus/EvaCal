...
export const SIEM_MONITORING_PRESET: IndustryPreset = {
  ...
  fields: [
    { label: 'Количество подключаемых источников событий', key: 'event_sources_count', type: 'number', required: true, order: 0 },
    { label: 'Количество площадок / ЦОД / филиальных узлов сбора', key: 'sites_count', type: 'number', required: true, order: 1 },
    { label: 'Средний поток событий (EPS)', key: 'eps_estimate', type: 'number', required: true, order: 2 },
    {
    ...
    { label: 'Средний размер одного события, байт', key: 'avg_event_size_bytes', type: 'number', required: true, order: 4 },
    { label: 'Оперативная ретенция (hot), суток', key: 'hot_retention_days', type: 'number', required: true, order: 5 },
    { label: 'Архивная ретенция, суток', key: 'archive_retention_days', type: 'number', required: true, order: 6 },
    { label: 'Ожидаемый коэффициент сжатия данных', key: 'compression_ratio', type: 'number', required: false, order: 7 },
    { label: 'Количество нестандартных источников / кастомных парсеров', key: 'custom_connectors_count', type: 'number', required: true, order: 8 },
    { label: 'Количество сценариев / правил корреляции к вводу', key: 'correlation_rules_count', type: 'number', required: true, order: 9 },
    { label: 'Количество внешних интеграций (IRP/SOAR/TI/Service Desk и др.)', key: 'siem_integrations_count', type: 'number', required: true, order: 10 },
    { label: 'Требуется резервирование коллекторов', key: 'collector_ha_required', type: 'checkbox', required: false, order: 11 },
    { label: 'Требуется HA критических компонентов SIEM', key: 'platform_ha_required', type: 'checkbox', required: false, order: 12 },
    { label: 'Целевая доступность SIEM, %', key: 'availability_target_percent', type: 'number', required: true, order: 13 },
    { label: 'Требуется взаимодействие с ГосСОПКА / НКЦКИ', key: 'gossopka_required', type: 'checkbox', required: false, order: 14 },
    {
    ...
    },
    {
      ...
      key: 'soc_model',
      type: 'select',
      options: ['Внутренний SOC', 'MSSP / внешний SOC', 'Гибридная модель', 'Только SIEM без отдельного SOC'],
      required: true,
      order: 16,
    },
    { label: 'Количество операторов и аналитиков SOC', key: 'soc_users_count', type: 'number', required: false, order: 17 },
    {
    ...
    { label: 'Сложность инфраструктуры мониторинга', key: 'complexity', type: 'complexity', required: true, order: 20 },
    { label: 'Дополнительные требования / ограничения', key: 'comment', type: 'textarea', required: false, order: 21 },
  ...
  riskTemplates: [
    {
      description: 'На части источников отсутствует требуемый аудит или журналы имеют нестабильный формат; потребуется доработка политик логирования и повторная приемка.',
      hours: 20,
      order: 0,
    },
    {
      description: 'Фактический средний или пиковый EPS выше пресейл-оценки; потребуется пересмотр вычислительных ресурсов, хранилища и лицензирования.',
      hours: 20,
      order: 1,
    },
    {
      description: 'Фактический размер и сжимаемость событий отличаются от исходных предположений, что изменит емкость hot/archive хранения.',
      hours: 12,
      order: 2,
    },
    {
      description: 'Высокий уровень ложных срабатываний потребует дополнительных итераций тюнинга правил корреляции и исключений.',
      hours: 20,
      order: 3,
    },
    {
      description: 'Нестандартные источники не имеют стабильного API/формата журналов и потребуют разработки и сопровождения кастомных коннекторов.',
      hours: 16,
      order: 4,
    },
  ],
