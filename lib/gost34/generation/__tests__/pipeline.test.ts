...
describe('Shared GOST preparation and export', () => {
  it.each(GOLDEN_SCENARIOS)('keeps preview and export equal for $id', async (scenario) => {
    const params = {
      ...
      metadataOverride: { docType: scenario.docType, standardProfileId: scenario.standardProfileId },
    };
  ...
  it.each([[], ['   '], ['1.1 ']].map((paragraphs) => ({ paragraphs })))('rejects an emptied mandatory section: $paragraphs', ({ paragraphs }) => {
    const params = { calculation, sectionOverrides: { 'tz2020-general': { paragraphs } } };
    const preview = prepareGost34Document(params, { mode: 'preview' });
    expect(preview.diagnostics.issues).toContainEqual(expect.objectContaining({
      nodeId: 'tz2020-general', kind: 'empty',
    }));
  ...
  it.each(['unknown', 'constructor', '__proto__'])('rejects unsupported type %s', (docType) => {
    expect(() => prepareGost34Document({ metadataOverride: { docType: docType as GostDocumentType } }))
  ...
  it('preserves actual metadata and ignores omitted overrides', () => {
    const { ast } = prepareGost34Document({
      ...
      metadataOverride: {
        ...
        signatures: { ...DEFAULT_SIGNATURES, developer: 'Автор', approver: 'Согласующий', signDate: '21.09.2026' },
      },
    });
  ...
  it('does not derive PDn, KII obligations or protection class from NGFW quantities', () => {
    const context = buildProjectContext({ answers: { ngfw_clusters_count: 2, vpn_tunnels_count: 5 } });
  ...
  it('includes funding details or explicitly reports the missing information', () => {
    ...
    const filled = prepareGost34Document({ calculation, projectContext: {
  ...
  it('keeps the LLM gate in export and suppresses invalid overlays in preview', () => {
    const tzAuthor = {
      promptVersion: 'tz-author-v1', speculateDefault: false,
      proposals: { 'tz2020-general': {
        nodeId: 'tz2020-general', schemaTitle: 'Общие сведения', status: 'ACCEPTED',
        paragraphs: ['Система должна соответствовать Приказу ФСТЭК России № 21.'],
        questions: [], flags: [], refusedGapPaths: [], speculate: false, usedLlm: true,
        provenance: { providerId: 'ollama', model: 'test', promptVersion: 'tz-author-v1', temperature: 0.2, createdAt: '2026-09-21', createdBy: 'test' },
      } },
    } as TzAuthorState;
