...
describe("assertAllowedEndpoint", () => {
  it("accepts a local model endpoint and strips the trailing slash", () => {
    expect(assertAllowedEndpoint("http://localhost:11434/")).toBe(
      "http://localhost:11434",
    );
    expect(assertAllowedEndpoint("http://127.0.0.1:1234/v1")).toBe(
      "http://127.0.0.1:1234/v1",
    );
  });

  it("accepts a remote provider over HTTPS", () => {
    expect(assertAllowedEndpoint("https://api.example.com/v1")).toBe(
  ...
  it("allows private addresses when the policy opts in", () => {
    ...
    expect(() =>
      assertAllowedEndpoint("http://169.254.169.254/", policy),
  ...
  it("rejects loopback when the policy forbids it", () => {
    ...
    expect(() =>
      assertAllowedEndpoint("http://localhost:11434", policy),
    ).toThrow(EndpointNotAllowedError);
  });

  it("rejects plain http for a remote host", () => {
    expect(() => assertAllowedEndpoint("http://api.example.com/v1")).toThrow(
      /HTTPS/,
    );
  });
  ...
  it.each([
    ...
    "ftp://example.com/",
    "not-a-url",
  ])("rejects the non-http(s) target %s", (endpoint) => {
    expect(() => assertAllowedEndpoint(endpoint)).toThrow(
      EndpointNotAllowedError,
    );
  });
  ...
  it("rejects credentials embedded in the URL", () => {
    expect(() =>
      assertAllowedEndpoint("https://user:pass@api.example.com/v1"),
    ).toThrow(EndpointNotAllowedError);
  });
});
