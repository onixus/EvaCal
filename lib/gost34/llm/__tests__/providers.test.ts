...
describe('built-in local providers', () => {
  it('ships Ollama and LM Studio out of the box', () => {
    expect(listLlmProviders().map((p) => p.id)).toEqual([OLLAMA_PROVIDER_ID, LMSTUDIO_PROVIDER_ID]);
  });

  it('honours OLLAMA_HOST / LMSTUDIO_HOST', () => {
    process.env.OLLAMA_HOST = 'http://127.0.0.1:9999/';
    expect(resolveLlmProvider(OLLAMA_PROVIDER_ID).endpoint).toBe('http://127.0.0.1:9999');
  });
...
describe('resolveLlmProvider', () => {
  ...
  it('never accepts a URL as the id', () => {
    expect(() => resolveLlmProvider('http://169.254.169.254')).toThrow(EndpointNotAllowedError);
  });
