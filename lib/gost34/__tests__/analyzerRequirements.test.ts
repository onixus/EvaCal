...
function analyze(enrich: boolean) {
  return analyzeAndNormalizeInput({
    ...
    metadataOverride: {
      ...
      enrichmentOptions: enrich
        ? { fstek_21: true, fstek_239: true }
        : undefined,
    },
  });
}
...
describe("analyzer requirement assembly", () => {
  ...
  it("attaches a validation report without blocking on library requirements", () => {
    ...
    expect(
      validation!.byRequirement["req-1"].some((f) => f.rule === "completeness"),
    ).toBe(true);
  ...
  it("appends enrichment after the project requirements", () => {
    ...
    expect(withEnrichment.slice(0, withoutEnrichment.length)).toEqual(
      withoutEnrichment,
    );
