...
export function buildPZ34Sections(
  ...
  const archStyle =
    ctx?.architecture?.style ||
    "Трехзвенная веб-архитектура (Client-Server-Database)";
  const componentsText =
    ctx?.architecture?.components?.join(", ") || "Модули системы";
  const platformsText =
    ctx?.infrastructure?.platforms?.join(", ") ||
    "Стандартные общесистемные платформы";
