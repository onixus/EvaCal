...
const schema: DocumentSchema = { id: 's', profileId: 'p', nodes: [{
  id: 'root', title: 'Root', required: true, children: [
    { id: 'a', title: 'A', required: true, children: [{ id: 'deep', title: 'Deep', required: true }] },
    { id: 'b', title: 'B', required: true },
    { id: 'optional', title: 'Optional' },
  ],
}] };
const section = (id: string, subsections?: Gost34Section[]): Gost34Section => ({
  id, title: id, numStr: '1', paragraphs: ['Text'], subsections,
});
...
describe('Recursive schema coverage', () => {
  ...
  it('accepts a populated tree without optional sections', () => {
    expect(validateSchemaCoverage(schema, [section('root', [
      section('a', [section('deep')]), section('b'),
    ])])).toEqual([]);
  });

  it('detects whitespace-only content and empty headings after editing', () => {
    const leafSchema: DocumentSchema = { id: 's', profileId: 'p', nodes: [schema.nodes[0].children![1]] };
    for (const edited of [{ ...section('b'), paragraphs: [' '] }, { ...section('b'), title: ' ' }]) {
  ...
  it('accepts table-only sections', () => {
    const leafSchema: DocumentSchema = { id: 's', profileId: 'p', nodes: [schema.nodes[0].children![1]] };
    expect(validateSchemaCoverage(leafSchema, [{ ...section('b'), paragraphs: [], tables: [
