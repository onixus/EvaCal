...
function getProjectDomainCorpus(context: ProjectContext): string {
  ...
  if (context.architecture?.components) parts.push(...context.architecture.components);
  if (context.security?.securityClass) parts.push(context.security.securityClass);
  return parts.join(' ');
}
...
export const APPLICABILITY_RULES: ApplicabilityRule[] = [
  // 1. Приказ ФСТЭК России № 21 (ИСПДн)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['fstek_21', 'фстэк_21', 'ispdn', 'испдн'])) {
        evidence.push({
          source: 'security.regulatoryScope',
          details: 'Приказ ФСТЭК № 21 явно указан в перечне нормативных актов системы',
      ...
      if (pdDataClass) {
        ...
        reasons.push('В составе обрабатываемых данных идентифицированы персональные данные.');
  ...
  // 2. 152-ФЗ / 242-ФЗ (Локализация баз ПДн в РФ)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['fz_152', '152-фз', '152_fz', '242-фз'])) {
        evidence.push({
          source: 'security.regulatoryScope',
          details: '152-ФЗ / 242-ФЗ явно указан в перечне нормативных актов системы',
          value: context.security?.regulatoryScope,
        });
  ...
  // 3. 187-ФЗ (О безопасности КИИ РФ)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['fz_187', '187-фз', '187_fz', 'kii', 'кии'])) {
        evidence.push({
          source: 'security.regulatoryScope',
  ...
  // 6. 188-ФЗ (Реестр отечественного ПО / Импортозамещение)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['fz_188', '188-фз', '188_fz', 'reestr', 'реестр'])) {
        evidence.push({
          source: 'security.regulatoryScope',
  ...
  {
    id: 'gost_57580',
    title: 'ГОСТ Р 57580.1-2017 / СТО БР ИББС (Безопасность финансовых операций)',
    category: 'security',
    evaluate: (context) => {
      const evidence: Evidence[] = [];
      const reasons: string[] = [];

      if (inRegulatoryScope(context, ['57580', 'gost_57580', 'сто бр', 'ibbs', 'иббс'])) {
        evidence.push({
          source: 'security.regulatoryScope',
  ...
  {
    id: 'cb_683p',
    title: 'Положение Банка России № 683-П (Безопасность ПО кредитных организаций)',
    category: 'security',
    evaluate: (context) => {
      const evidence: Evidence[] = [];
      const reasons: string[] = [];
      ...
      if (inRegulatoryScope(context, ['683-п', '683_p', 'cb_683p', '683п'])) {
        ...
        reasons.push('Положение Банка России № 683-П включено в нормативный скоуп.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 1.0 };
      }
      ...
      if (/кредитн\w*\s+организаци|банк\w*/i.test(corpus)) {
        ...
        reasons.push('Заказчик или объект автоматизации является кредитной организацией.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 0.85 };
      }
  ...
  // 9. Положение Банка России № 757-П (НФО)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['757-п', '757_p', 'cb_757p', '757п'])) {
        ...
        reasons.push('Положение Банка России № 757-П включено в нормативный скоуп.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 1.0 };
      }
      ...
      if (/нфо|некредитн\w*\s+финансов|страхов\w*\s+компан|брокер|микрофинанс|мфо|негосударственн\w*\s+пенсионн|депозитари/i.test(corpus)) {
        ...
        reasons.push('Организация относится к некредитным финансовым организациям (НФО).');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 0.85 };
      }

      reasons.push('Принадлежность к некредитным финансовым организациям не подтверждена.');
      return { status: 'UNKNOWN', reasons, evidence, confidence: 0.0 };
    },
  },
  ...
  // 10. Положение Банка России № 719-П (Антифрод и электронная подпись)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['719-п', '719_p', 'cb_719p', '719п'])) {
        ...
        reasons.push('Положение Банка России № 719-П включено в нормативный скоуп.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 1.0 };
      }
      ...
      reasons.push('Необходимость процедур антифрода и требований 719-П не подтверждена.');
      return { status: 'UNKNOWN', reasons, evidence, confidence: 0.0 };
    },
  },
  ...
  // 11. Приказ ФСБ России № 282 (ГосСОПКА / НКЦКИ)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['fsb_282', 'фсб_282', 'gossopka', 'госсопка', 'нкцки'])) {
        evidence.push({
          source: 'security.regulatoryScope',
          details: 'Взаимодействие с ГосСОПКА по Приказу ФСБ № 282 включено в скоуп проекта',
          value: context.security?.regulatoryScope,
        });
        reasons.push('Требования взаимодействия с ГосСОПКА включены в нормативный скоуп.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 1.0 };
      }
  ...
  {
    id: 'sla_999',
    title: 'SLA 99.9% (Непрерывность функционирования и RTO ≤ 15 мин, RPO ≤ 5 мин)',
    ...
    evaluate: (context) => {
      ...
      if (target !== undefined && target >= 99.9) {
        ...
        reasons.push(`Зафиксирован высокий целевой показатель доступности: ${target}%.`);
        return { status: 'APPLICABLE', reasons, evidence, confidence: 0.95 };
      }
      ...
      if (target !== undefined && target < 99.0 && (rto === undefined || rto > 60)) {
        evidence.push({
          source: 'availability',
      ...
      reasons.push('Требования к SLA 99.9% и целевым метрикам RTO/RPO не зафиксированы.');
      return { status: 'UNKNOWN', reasons, evidence, confidence: 0.0 };
    },
  },
  ...
  // 13. ГОСТ Р 52872-2019 / WCAG 2.1 AA (Доступность веб-интерфейсов)
  {
    ...
    evaluate: (context) => {
      ...
      if (inRegulatoryScope(context, ['wcag', '52872', 'доступность', 'accessibility'])) {
        evidence.push({
          source: 'security.regulatoryScope',
          details: 'Требования доступности ГОСТ Р 52872-2019 явно включены в скоуп проекта',
          value: context.security?.regulatoryScope,
        });
        reasons.push('Требования доступности веб-интерфейсов включены в нормативный скоуп.');
        return { status: 'APPLICABLE', reasons, evidence, confidence: 1.0 };
      }
