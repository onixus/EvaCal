...
describe('round trip', () => {
  ...
  it('preserves an array', () => {
    const back = toGost34RequirementItems(fromGost34RequirementItems([fullItem]));
...
describe('fromGost34RequirementItem', () => {
  ...
  it('defaults to DRAFT and infers the requirement type from the code', () => {
    ...
    expect(fromGost34RequirementItem({ ...fullItem, code: 'ТР-ГОСТ-01' }).type).toBe('regulatory');
  });
...
describe('toGost34RequirementItem', () => {
  ...
  it('uses the original text for an unapproved requirement by default', () => {
    expect(toGost34RequirementItem(proposed).description).toBe('исходная формулировка');
  });
