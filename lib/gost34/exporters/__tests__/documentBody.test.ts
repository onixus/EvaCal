...
const section = (id: string, numStr: string, subsections?: Gost34Section[]): Gost34Section => ({
  id, numStr, title: id, paragraphs: ['Text'], subsections,
});

it('preserves level-three heading styles in Word', async () => {
  const { ast } = prepareGost34Document({ metadataOverride: { layoutProfileId: 'plain-corporate' } });
  ast.sections = [section('Root', '1', [section('Child', '1.1', [section('Grandchild', '1.1.1')])])];
...
describe('Independent table counters', () => {
  it.each(['gost34-modern', 'gost34-eskd-frame', 'plain-corporate'] as const)(
    'restarts numbering for each appendix in %s', async (layoutProfileId) => {
