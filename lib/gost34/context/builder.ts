...
function record(state: BuildState, path: string, source: ContextSource, evidence?: string) {
...
export function buildProjectContext(input: ProjectContextInput): ProjectContext {
  ...
  const screensCount = screensAnswer ? toNumber(screensAnswer.value) : undefined;
  ...
  const integrationsAnswer = findAnswer(answers, /integrations?|интеграц|смежн/i);
  const integrationsList = integrationsAnswer ? toList(integrationsAnswer.value) : undefined;
  const integrationsCount = integrationsAnswer ? toNumber(integrationsAnswer.value) : undefined;
  ...
  if (importValue !== undefined) {
    ...
    record(state, 'infrastructure.importSubstitution', 'questionnaire', importAnswer!.key);
  }

  if (Object.keys(infrastructure).length > 0) ctx.infrastructure = infrastructure;

  // ── Доступность и производительность ─────────────────────────────────
  const availabilityAnswer = findAnswer(answers, /availability|доступност|sla/i);
  ...
  if (responseAnswer) {
    ...
    record(state, 'performance.maxResponseTimeMs', 'questionnaire', responseAnswer.key);
  }
  ...
  if (pdnValue !== undefined) {
    ...
    record(state, 'security.personalDataProcessed', 'questionnaire', pdnAnswer!.key);
  }
