...
function makeRequirement(
  overrides: Partial<Gost34RequirementV2> = {},
): Gost34RequirementV2 {
  return {
    id: "req-1",
    code: "ТР-ФУНК-01",
...
describe("atomicity", () => {
  ...
  it("flags enumerations inside a single requirement", () => {
    const report = validateRequirement(
      makeRequirement({
        originalText:
          "Система должна поддерживать роли: администратор; аудитор; оператор.",
      }),
    );
...
describe("ambiguity", () => {
  it("flags evaluative wording", () => {
    const report = validateRequirement(
      makeRequirement({
        originalText:
          "Система должна работать быстро и быть удобной для оператора.",
      }),
    );
...
describe("measurability", () => {
  it("reports a performance requirement without a number as an error", () => {
    const report = validateRequirement(
      makeRequirement({
        ...
        originalText:
          "Система должна обеспечивать высокую пропускную способность.",
      }),
    );
  ...
  it("accepts a performance requirement with a measured value", () => {
    const report = validateRequirement(
      makeRequirement({
        category: "performance",
        originalText:
          "Система должна обрабатывать не менее 500 запросов в секунду.",
      }),
    );
  ...
  it("demands a value once a named indicator appears in any category", () => {
    const report = validateRequirement(
      makeRequirement({
        originalText:
          "Система должна обеспечивать срок хранения журналов аудита.",
      }),
    );
...
describe("completeness", () => {
  ...
  it("rejects a requirement without a code", () => {
    ...
    expect(find(report.findings, "completeness")?.message).toContain(
      "обозначения",
    );
  });
});
...
describe("source", () => {
  ...
  it("warns when the source is only a hash", () => {
    const report = validateRequirement(
      makeRequirement({ source: { hash: "abc123" } }),
    );
  ...
  it("reports library requirements as INFO and never blocks on them", () => {
    const report = validateRequirement(
      makeRequirement({
        ...
        source: undefined,
        originalText:
          "Система должна обеспечивать защиту персональных данных надёжным образом.",
        verificationMethod: undefined,
      }),
    );
  ...
  it("leaves consistent values alone", () => {
    const report = validateRequirements([
      ...
      makeRequirement({
        ...
        category: "performance",
        originalText:
          "Время отклика системы должно составлять не более 2 с при 100 пользователях.",
      }),
    ]);
...
describe("report shape", () => {
  ...
  it("honours disabled rules", () => {
    ...
    expect(rules(validateRequirement(requirement).findings)).toContain(
      "ambiguity",
    );
    expect(
      rules(
        validateRequirement(requirement, { rules: { ambiguity: false } })
          .findings,
      ),
  ...
  it("formats a finding as one line", () => {
    ...
    expect(formatValidationFinding(report.findings[0])).toContain(
      "Рекомендация:",
    );
  });
});
