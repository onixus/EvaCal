...
const QUANTIFIABLE_CATEGORIES: RequirementCategory[] = ['performance', 'reliability'];
...
function needsNumericValue(check: RequirementCheck): boolean {
  ...
  return CONFLICT_INDICATORS.some((indicator) => indicator.pattern.test(check.text));
}
 ...
 */
export function checkMeasurability(check: RequirementCheck): ValidationFinding[] {
