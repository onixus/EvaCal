...
const MISSING_SUBJECT_PATTERN =
  /^(?:должн[а-яё]*|должен|обязан[а-яё]*|необходимо|следует|требуется|обеспеч[а-яё]+)/iu;
  ...
  if (!check.hasModal) {
    findings.push(
      finding(
        ...
        'completeness',
        'ERROR',
        'Текст не выражен как требование: отсутствует обязывающая формулировка («должен», «должна», «обязан»).',
