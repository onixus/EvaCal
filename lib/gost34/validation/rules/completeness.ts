...
const MISSING_SUBJECT_PATTERN = /^(?:должн[а-яё]*|обязан[а-яё]*|необходимо|следует|требуется|обеспеч[а-яё]+)/iu;
...
export function checkCompleteness(check: RequirementCheck): ValidationFinding[] {
  ...
  if (!requirement.code?.trim()) {
    findings.push(
      finding(check, 'completeness', 'ERROR', 'У требования нет обозначения (кода), на него нельзя сослаться.', 'Присвоить код вида «ТР-ФУНК-01».')
    );
  }

  if (!text) {
    findings.push(
      finding(check, 'completeness', 'ERROR', 'Текст требования пуст.', 'Заполнить формулировку требования или удалить запись.')
    );
    return findings;
  }
  ...
  if (!requirement.title?.trim()) {
    findings.push(
      finding(check, 'completeness', 'WARNING', 'У требования нет заголовка.', 'Добавить краткий заголовок для оглавления и таблиц трассируемости.')
    );
  }
